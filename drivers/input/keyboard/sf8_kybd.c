...
static int sf8_kybd_irqsetup(void)
{
    ...
    rc = request_any_context_irq(PM8058_GPIO_IRQ(PMIC8058_IRQ_BASE, rd->pmic_gpio_vol_up), &sf8_kybd_irqhandler,
    ...
    rc = request_any_context_irq(PM8058_GPIO_IRQ(PMIC8058_IRQ_BASE, rd->pmic_gpio_vol_dn), &sf8_kybd_irqhandler,
                 (IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING), 
                 sf8_kybd_name, rd);
    if (rc < 0) {
        dev_err(rd->sf8_kybd_pdev,
    ...
    rc = request_any_context_irq(PM8058_GPIO_IRQ(PMIC8058_IRQ_BASE, rd->pmic_gpio_cover_det), &sf8_kybd_irqhandler,
                 (IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING), 
                 sf8_kybd_name, rd);
    if (rc < 0) {
        dev_err(rd->sf8_kybd_pdev,
